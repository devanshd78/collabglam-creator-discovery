...
export async function getUnitsUsedToday(): Promise<number> {
  const row = await prisma.youtubeApiUsage.findUnique({
    where: {
      date: todayKey(),
    },
  });
...
/**
 * YouTube Data API default quota is 10,000 units per API key/day.
 * If 10 keys are configured, the dashboard displays a 100,000-unit
 * combined team budget.
 */
export function dailyUnitBudget(): number {
  const keys = String(process.env.YOUTUBE_API_KEY ?? "")
    .split(",")
    .map((key) => key.trim())
    .filter(Boolean);

  return keys.length * 10_000;
}